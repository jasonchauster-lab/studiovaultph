...
function LoginContent() {
    ...
    const isRoleLocked = searchParams.has('role') || searchParams.has('plan') || !!studioSlug
    ...
    const [studioData, setStudioData] = useState<{ name: string, logo_url: string | null } | null>(null)
    ...
    const returnHomeUrl = studioSlug ? `/s/${studioSlug}` : (plan ? '/pricing' : '/')
    ...
    // ── 0. Handle Hash Errors (Supabase Implicit Flow) ──────────────────
    useEffect(() => {
        if (typeof window !== 'undefined' && window.location.hash) {
            ...
            if (error) {
                let friendlyMessage = decodeURIComponent(error).replace(/\+/g, ' ')
                
                // Make common Supabase errors more user-friendly
                if (errorCode === 'otp_expired') {
                    friendlyMessage = 'This verification link has expired or has already been used. Please request a new one.'
                } else if (errorCode === 'access_denied') {
                    friendlyMessage = 'Access denied. Please try signing in again.'
                }
    ...
    // 0. Fetch studio branding if scoped
    useEffect(() => {
        if (!studioSlug) return
        const fetchStudioBranding = async () => {
            const { data, error } = await supabase
                .from('studios')
                .select('name, logo_url, website_config')
                .eq('slug', studioSlug)
                .maybeSingle()
            
            if (data && !error && mounted.current) {
                // Try to get header logo first from config
                const configLogo = data.website_config?.header?.logoUrl
                setStudioData({
                    name: data.name,
                    logo_url: configLogo || data.logo_url
                })
            }
        }
        fetchStudioBranding()
    ...
    useEffect(() => {
        const checkSession = async () => {
            ...
            if (session?.user && !isRedirecting) {
                ...
                if (isVerified) {
                   if (mounted.current) setIsRedirecting(true)
                   await redirectByRole(session.user.id)
                } else {
                    // Passed password but not 2FA -> jump to OTP step
                    // We ONLY jump to OTP if we aren't already showing an error (like otp_expired)
                    if (mounted.current) setStep('otp')
                    if (session.user.email && mounted.current) setEmail(session.user.email)
                    ...
                    if (rememberMePref === '1' && mounted.current) setRememberDevice(true)
                }
            }
        }
        checkSession()
    ...
    const redirectByRole = async (userId: string) => {
        try {
            ...
            if (isStudioPortal) {
                ...
                // 2. Customers stay on the storefront or go to their customer dashboard
                else if (profile?.role === 'customer') {
                    // If we have a studio slug in the URL, go back to that storefront
                    if (studioSlug) {
                        router.push(`/s/${studioSlug}`)
                    } else {
                        // Fallback to customer portal
                        router.push('/customer')
                    }
                }
                // 3. Studio Owners go to /studio
                else {
                    router.push('/studio')
                }
                router.refresh()
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        if (isSignUp) {
            ...
            if (existingProfile) {
                setMessage({ type: 'error', text: 'An account with this email already exists. Your existing StudioVault account works here — please sign in instead!' })
                setLoading(false)
                return
            }
            ...
            const { error } = await supabase.auth.signUp({
                ...
                options: {
                    data: { 
                        ...
                        role: isStudioPortal ? 'studio' : role, 
                    ...
                    emailRedirectTo: `${window.location.origin}/auth/callback?next=confirm${refCode ? '&ref=' + refCode : ''}${plan ? '&plan=' + plan : ''}${billing ? '&billing=' + billing : ''}`
                }
            })
            if (error) {
    ...
    return (
        <div className="min-h-screen bg-off-white flex flex-col md:flex-row relative selection:bg-forest/10">
                    ...
                    <div className="earth-card p-12 border-border-grey/50 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.12)]">
                        <div className="space-y-12">
                            ...
                            <p className="text-xl text-charcoal-700 font-medium leading-relaxed italic border-l-4 border-burgundy/20 pl-6">
                                &ldquo;Experience a platform designed with the precision and grace of {studioData?.name || 'Pilates'} itself.&rdquo;
                            </p>
            ...
            <main className="flex-1 flex flex-col items-center justify-center p-6 sm:p-10 md:p-12 lg:p-24 overflow-y-auto relative bg-off-white pb-16 md:pb-24">
                <Link href={returnHomeUrl} className="hidden md:flex absolute top-12 left-12 items-center gap-3 text-slate-600 hover:text-charcoal transition-all group">
                ...
                <div className="w-full max-w-md pt-10 sm:pt-0">
                    <div className="flex flex-col items-center mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
                        <Link href={returnHomeUrl} className="flex flex-col items-center group">
                            <Image src={studioData?.logo_url || "/logo4.png"} alt={`${studioData?.name || 'StudioVault'} Logo`} width={240} height={80} className="h-20 w-auto object-contain" />
                        </Link>
                    </div>
                    ...
                    {step === 'otp' ? (
                        ...
                        <>
                            ...
                            {isSignUp && !isRoleLocked && (
                                <div role="radiogroup" aria-label="Selection Role" className="mb-10 p-1 bg-white rounded-xl flex gap-1 border border-border-grey shadow-tight">
                                    {[{ id: 'customer', label: 'Client' }, { id: 'instructor', label: 'Instructor' }, { id: 'studio', label: 'Studio' }].map((opt) => (
                                        <button key={opt.id} type="button" role="radio" aria-checked={role === opt.id} onClick={() => setRole(opt.id)}
                                            className={`flex-1 py-4 px-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all duration-300 ${role === opt.id ? 'bg-forest text-white shadow-md' : 'text-slate-600 hover:text-charcoal hover:bg-off-white hover:shadow-sm'}`}>
                                            {opt.label}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <button type="button" onClick={handleGoogleAuth} disabled={loading}
                                className="w-full flex items-center justify-center gap-3 px-5 h-14 border border-charcoal/20 bg-white rounded-xl text-[13px] font-bold uppercase tracking-widest text-charcoal hover:bg-off-white hover:border-charcoal/40 hover:shadow-md transition-all shadow-tight disabled:opacity-50 mb-12 active:scale-[0.98] cursor-pointer">
                            ...
                            <form onSubmit={handleAuth} className="w-full space-y-4">
                                ...
                                <div className="text-center pt-8">
                                    <button type="button"
                                        ...
                                        className="text-slate-600 hover:text-charcoal text-[11px] font-bold uppercase tracking-[0.3em] transition-all group p-2">
                                        {isSignUp
                                            ? <>Already have an account? <span className="text-burgundy border-b-2 border-burgundy/30 group-hover:border-burgundy transition-all pb-1 ml-1">Log in.</span></>
                                            : <>Don&apos;t have an account? <span className="text-burgundy border-b-2 border-burgundy/30 group-hover:border-burgundy transition-all pb-1 ml-1">Sign up.</span></>
                                        }
                                    </button>
                                </div>
                            </form>
