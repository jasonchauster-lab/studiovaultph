...
export default async function BranchMembershipsPage(props: {
    ...
    // 3. Fetch all active memberships and categories
    const [
        { data: rawMemberships }, 
        { data: categories }
    ] = await Promise.all([
        supabase.from('memberships').select('*').eq('studio_id', studio.id).eq('is_private', false).order('price', { ascending: true }),
    ...
    const allPlans = (memberships || []).map(m => ({ ...m, type: 'membership' }))
    ...
    if (uncategorizedItems.length > 0) {
        groupedData.push({
            ...
            description: 'Standard membership plans and recurring passes.',
            ...
            <main className="flex-1 pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto w-full">
                <div className="mb-20 space-y-6">
                    ...
                    <p className="text-zinc-500 max-w-2xl leading-relaxed">
                        Choose a recurring plan that fits your lifestyle. Our memberships offer the best value for consistent practice.
                    </p>
                </div>
                ...
                <div className="space-y-12">
                    {groupedData.length > 0 ? (
                        ...
                        <div className="py-20 text-center bg-zinc-900/5 rounded-[3rem] border border-dashed border-zinc-200">
                             <p className="text-zinc-400 italic">No memberships available for this location yet.</p>
                        </div>
                    )}
                </div>
            </main>
