...
export default async function PackagesRedirector(props: {
    ...
    if (outlets.length === 0) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <p className="text-zinc-500 font-medium tracking-tight">Packages currently unavailable.</p>
            </div>
        )
    }
    ...
    if (preferredOutletSlug) {
        ...
        if (preferred) {
            redirect(`/s/${slug}/${preferred.slug}/packages${queryString}`)
        }
    }

    // 3. Default to first branch
    redirect(`/s/${slug}/${outlets[0].slug}/packages${queryString}`)
}
