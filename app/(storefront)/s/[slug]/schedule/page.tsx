...
export default async function ScheduleRedirector(props: {
    ...
    if (outlets.length === 0) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <p className="text-zinc-500 font-medium tracking-tight">Schedule currently unavailable.</p>
            </div>
        )
    }
    ...
    if (preferredOutletSlug) {
        ...
        if (preferred) {
            redirect(`/s/${slug}/${preferred.slug}/schedule${queryString}`)
        }
    }

    // 3. Default to first branch if only one exists or no preference
    redirect(`/s/${slug}/${outlets[0].slug}/schedule${queryString}`)
}
