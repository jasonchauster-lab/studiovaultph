...
import { formatManilaDateStr, toManilaDateStr, toManilaTimeString, formatTo12Hour } from '@/lib/timezone'
...
export async function bookSlot(slotId: string, equipment: string, quantity: number = 1) {
    ...
    for (let i = 0; i < allocatedSlotIds.length; i++) {
        ...
        const priceForThisSlot = i === 0 ? finalPrice : 0;
        ...
        if (rpcResult?.success) {
            ...
            if (!bookingRecordId) {
                bookingRecordId = rpcResult.booking_id;
            }
        }
    }
