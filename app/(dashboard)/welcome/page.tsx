            ...
            <div className="max-w-4xl w-full text-center mt-auto mb-auto flex flex-col gap-y-10">
                ...
                <div className="flex flex-col items-center gap-y-3">
                    <h1 className="text-4xl md:text-5xl font-serif text-burgundy tracking-tight">
                        {isStudioPortal ? 'Register your Studio' : 'Welcome to Studio Vault'}
                    </h1>
                    <p className="text-muted-burgundy text-lg leading-relaxed max-w-md">
                        {isStudioPortal 
                            ? "To access the Management Portal, you'll need to set up your studio profile first."
                            : 'How will you be using the platform today?'}
                    </p>
                </div>

                <div className={isStudioPortal ? "flex justify-center" : "grid grid-cols-1 md:grid-cols-2 gap-6"}>
                    {/* Instructor Card */}
                    {!isStudioPortal && (
                        <form action={async () => {
                            'use server'
                    ...
                    <form 
                        ...
                        className={isStudioPortal ? "max-w-md w-full" : ""}
                    >
                        <button className="w-full h-full group bg-white p-8 rounded-2xl border-2 border-border-grey shadow-tight hover:shadow-card hover:border-burgundy/30 transition-all flex flex-col items-center gap-y-4">
                            <div className="w-14 h-14 bg-off-white rounded-full flex items-center justify-center group-hover:bg-burgundy transition-colors duration-300 border border-border-grey">
                            ...
                            <div className="flex flex-col gap-y-1">
                                <h2 className="text-xl font-serif text-burgundy">
                                    {isStudioPortal ? 'Create Studio Profile' : 'I own a Studio'}
                                </h2>
                                <p className="text-sm text-muted-burgundy leading-relaxed">
                                    {isStudioPortal 
                                        ? 'Launch your studio dashboard and start managing your space.'
                                        : 'List your studio, manage availability, and get paid.'}
                                </p>
                            </div>
                        </button>
                    </form>
                ...
                {isStudioPortal && (
                    <div className="mt-8 flex flex-col gap-y-2">
                        <p className="text-xs text-muted-burgundy opacity-60 uppercase font-black tracking-widest">
                            Not a studio owner?
                        </p>
                        <a 
                            href="https://studiovaultph.com" 
                            className="text-sm text-burgundy font-bold underline hover:opacity-80 transition-opacity"
                        >
