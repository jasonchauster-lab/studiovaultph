...
export default function Navigation({ role }: NavigationProps) {
    ...
    const navLinks = (
        <>
            {isCustomerMode ? (
                <>
                    ...
                    >
                        ...
                        Referral
                        {pathname === '/referral' && <span className="absolute -bottom-3 left-0 w-full h-0.5 bg-forest rounded-full" />}
                    </Link>
                    ...
                    {role === 'instructor' && (
                        <>
                            ...
                            <Link href="/referral" onClick={() => setMobileOpen(false)} className={linkClass(pathname === '/referral')}>
                                <Gift className="w-3.5 h-3.5" />
                                Referral
                                {pathname === '/referral' && <span className="absolute -bottom-2 left-0 w-full h-0.5 bg-forest rounded-full" />}
                            </Link>
            ...
            <div className="md:hidden pt-4 border-t border-border-grey mt-4">
                <form action={signOut}>
                    <button className="flex items-center gap-2 text-xs uppercase tracking-widest font-bold text-red-600 hover:text-red-700 transition-colors py-2">
