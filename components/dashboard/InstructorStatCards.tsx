        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-0">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8 sm:mb-10">
                {cardData.map((card, index) => (
                    <div key={index} className="earth-card p-3.5 sm:p-6 group relative overflow-hidden transition-all duration-500 hover:-translate-y-1 shadow-tight border border-border-grey/40 bg-white/60 backdrop-blur-sm">
                        ...
                        <div className="flex justify-between items-start mb-3 sm:mb-5 relative z-10">
                            <div className={clsx(
                                "p-2 sm:p-2.5 rounded-lg sm:rounded-xl group-hover:scale-105 transition-all duration-500 border border-border-grey/40 shadow-sm bg-white/40",
                                card.bgIcon
                            )}>
                                <card.icon className={clsx("w-3.5 h-3.5 sm:w-4 sm:h-4", card.iconColor)} />
                            </div>
                            <span className={clsx(
                                "text-[7px] sm:text-[9px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] px-2 sm:px-3 py-1 sm:py-1.5 rounded-md sm:rounded-lg border shadow-sm transition-all",
                        ...
                        <div className="relative z-10 space-y-0.5 sm:space-y-1">
                            <p className="text-[9px] sm:text-[9px] font-black text-charcoal/40 uppercase tracking-[0.2em] sm:tracking-[0.4em] leading-tight mb-1 sm:mb-2">{card.label}</p>
                            <h3 className="text-xl sm:text-3xl font-serif text-charcoal tracking-tighter truncate leading-none">{card.value}</h3>
                        </div>
    
                        {card.hasAction && (
                            <div className="mt-4 sm:mt-6 pt-3 sm:pt-5 border-t border-border-grey/40 relative z-10">
                                <Link
