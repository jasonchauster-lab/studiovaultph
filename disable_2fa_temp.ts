...
async function disableAuthShield() {
  for (const email of emails) {
    ...
    try {
        const { data: factors, error: mfaError } = await (supabase.auth.admin as any).mfa.listFactors({ userId: user.id })
        if (!mfaError && factors && factors.factors.length > 0) {
            ...
            for (const factor of factors.factors) {
                await (supabase.auth.admin as any).mfa.deleteFactor({ userId: user.id, factorId: factor.id })
            }
