-- Migration: 20260408_strict_studio_isolation.sql

-- 1. Add studio_id to wallet_top_ups to isolate manual adjustments/top-ups
ALTER TABLE public.wallet_top_ups ADD COLUMN IF NOT EXISTS studio_id UUID REFERENCES public.studios(id);

-- 2. Update get_studio_earnings_v2 to be strictly studio-scoped
...
BEGIN
    ...
    -- 2. Owner Balance (Still global for now, but we only show studio-specific history)
    ...
    RETURN (
        ...
        all_tx AS (
            ...
            SELECT 
                ...
                'studio' as origin
            FROM wallet_top_ups
            WHERE studio_id = p_studio_id -- STRICT ISOLATION: Must have studio_id set
            AND status = 'approved'
            AND (p_start_date IS NULL OR created_at::DATE >= p_start_date)
            AND (p_end_date IS NULL OR created_at::DATE <= p_end_date)
        )
