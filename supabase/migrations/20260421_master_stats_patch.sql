-- Master Patch: Correcting Aliases and Hardening Authorization
-- This fixes the "column pb does not exist" error and ensures RLS transparency.

CREATE OR REPLACE FUNCTION get_studio_dashboard_stats_v4(
    p_studio_id UUID, 
...
BEGIN
    ...
    -- DATA FETCHING (Fixed b alias and metadata path)
    SELECT COALESCE(SUM((b.metadata->>'studio_fee')::NUMERIC), 0) INTO v_revenue
