-- Fixes the "FOR UPDATE cannot be applied to the nullable side of an outer join" error
-- by explicitly locking only the bookings table.

-- 1. Atomic RPC for processing booking completion to avoid race conditions
...
BEGIN
    ...
    FOR UPDATE OF b; -- Only lock the bookings row to avoid LEFT JOIN error
...
-- 2. Atomic RPC for unlocking matured funds
